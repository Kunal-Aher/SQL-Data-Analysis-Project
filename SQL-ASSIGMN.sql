...
alter table history add primary key(empcode,changedate,desigcode,gradecode,gradelevel,deptcode);
...
from emp having age > 55 order by age; 
...
SELECT e.empcode, e.empname, d.deptcode, d.dept_name, de.designame 
FROM emp e 
JOIN dept d ON e.deptcode = d.deptcode 
JOIN desig de ON e.desigcode = de.desigcode 
...
select e.empcode,e.empname,d.deptcode,d.dept_name 
from emp e join dept d on e.deptcode = e.deptcode 
where dept_name IN ('accounts'AND'personal') order by dept_name ;
-- or
SELECT e.empcode, e.empname, d.deptcode, d.dept_name
FROM emp e
JOIN dept d ON e.deptcode = d.deptcode
WHERE d.dept_name IN ('accounts','personal')
...
select e.empcode,e.empname,d.deptcode,d.dept_name
from emp e join dept d on e.deptcode = e.deptcode 
where dept_name='accounts'and dept_name <> 'personal';
...
select empcode,empname,timestampdiff(year,joindate,curdate()) 
as experience from emp having experience > 25 order by experience ; 


-- 21. List the ‘Salesmen’ who have minimum 30 to 20 years of experience
SELECT e.empcode, e.empname, d.designame, d.desigcode, TIMESTAMPDIFF(YEAR, e.joindate, CURDATE()) AS experience
FROM emp e 
...
select e.empcode,e.empname,s.basic,s.basic/2 as basic2 from emp e 
...
from emp e join dept d on e.deptcode=d.deptcode 
having d.dept_name='Accounts' order by empname and age;

-- 27. List the number of employees directly reporting to ‘Reddy’
select count(*) as Reddy_Supervision from emp 
...
from grade a 
join emp e 
join salary s on a.gradecode =e.gradecode and e.empcode = s.empcode 
...
select e.empcode,e.empname,e.supcode ,count(g.gradecode) 
as Promotion_count,g.gradelevel,e.basicpay 
from emp e 
join grade g 
on e.gradelevel = g.gradelevel 
group by e.empcode,e.empname having count(g.gradecode) order by basicpay Desc,Promotion_count; 
-- 35. List the employees who got promoted in the year 1991.
select e.empcode,e.empname,h.changedate as Promoted
from emp e inner join history h on e.empcode=h.empcode 
having Promoted between '1991-01-01'AND  '1991-12-31' ;
...
as Drawn_salary 
from emp e join salary s join  dept d on d.deptcode = e.deptcode and e.empcode=s.empcode 
group by e.empcode,e.empname order by budget,Drawn_salary ;
...
-- group by e.empcode,e.empname
having morethenavg_pay > (select avg(s.basic + s.allow - s.deduct) from salary s ) 
order by morethenavg_pay ASC; 
...
from emp e 
join salary s on e.empcode=s.empcode group by e.empcode, e.empname 
order by highest_salary Desc limit 2;
...
join salary s ON e.empcode = s.empcode
group by e.empcode,e.empname 
having salaries < (select max(s.basic + s.allow + s.deduct) from salary s) 
ORDER BY salaries DESC LIMIT 1 OFFSET 1;
...
join salary s on d.deptcode = e.deptcode 
and e.empcode=s.empcode group by e.empname,s.empcode having sex='F';
...
select m.empcode, m.empname, m.desigcode, m.basicpay as Manager_Salary, d.deptcode, d.dept_name
from emp m
join dept d on m.deptcode = d.deptcode
where d.deptcode = (
    select e.deptcode
    from emp e
    join dept d on e.deptcode = d.deptcode
...
select e.empcode,e.empname,d.deptcode,d.dept_name,e.basicpay from emp e 
join dept d on e.deptcode = d.deptcode group by e.empcode,e.empname 
having min(basicpay) < (select avg(basicpay) from emp);
...
select empname,timestampdiff(year,joindate,curdate()) as 
EMP_Experince from emp order by EMP_Experince asc ;
...
from emp e join salary s on e.empcode = s.empcode group by e.empcode,e.empname 
order by current_d desc;
...
where w.EMP_AGE > (select max(timestampdiff(year,e.joindate,curdate())) 
from emp where desigcode="SPRV") ;
...
select d.dept_name, (e.NO_of_emp_in_Dept*100 / t.NO_of_emp_in_organization ) 
as percentage_of_employee_in_each_department from dept d join Total_emp t 
join NO_of_Emp e on 1=1 and d.dept_name=e.dept_name ;
